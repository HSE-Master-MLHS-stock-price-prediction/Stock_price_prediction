...
class SimplePortfolioExperiment:
    ...
    def __init__(self,
        ...
        assert 0.0 <= stock_start_partial <= 1.0
        ...
        if historical_data is None:
            self.historical_data = list(
                yf.Ticker('YNDX').history(period="max")['Close'][1412:-250])
        else:
            self.historical_data = historical_data

        self.number_of_stocks = int(
            self.start_capital * stock_start_partial // self.historical_data[
                0])
        self.free_money = self.start_capital - self.number_of_stocks * \
                          self.historical_data[0]

        self.history_capital = [
            self.free_money + self.number_of_stocks * self.historical_data[0]]
    ...
    def sell_papers(self, number_of_papers, ind, partial_operation=False):
        if self.number_of_stocks < number_of_papers:
            if partial_operation:
                money_to_add = self.historical_data[
                                   ind] * self.number_of_stocks
        ...
        if money > self.free_money:
            if partial_operation:
                number_of_papers = int(
                    self.free_money // self.historical_data[ind])
    ...
    def start_experiment(self):
        ...
        for i in tqdm(range(1, len(self.historical_data))):
            ...
            if i % self.days_per_action == 0 and self.window_size <= i:
                self.make_action(i)
            self.history_capital.append(
                self.free_money + self.number_of_stocks * self.historical_data[
                    i])
